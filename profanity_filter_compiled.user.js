...
let a=new RegExp("\\b(?:(?:f(?:a(?:g(?:(?:g[eio]|o)t)?|tass)|u(?:ck(?:me)??|k)|cuk|eg)|b(?:u(?:llshit|ttfuck)|[!i]tch|astard)|ass(?:(?:hol|wip)e|clown|fuck|kiss)??|d(?:ipshit|umbass|ouche|amn)|p(?:hu(?:c?k|q)|iss(?:off)??)|sh(?:i(?:t(?:full)??|z)|!t)|moth(?:afucka?|erfuck)|c(?:rap|unt)|nig(?:ga)??|goddamn|jackass))[tgk]??(?:in(?:g)?|ed|(?:i)??er)??(?:(?:[yz]|e?s))??\\b","gi"),b=document.evaluate("./*[not(self::script or self::noscript or self::code or self::textarea)]//text()[string-length(normalize-space()) > 2]",
document.body,null,XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE,null);if(a.test(document.title))document.title=document.title.replace(a,"*bleep*");for(let c=0,d=b.snapshotItem(0);null!==d;){if(a.test(d.data))d.data=d.data.replace(a,"*bleep*");d=b.snapshotItem(++c);}
